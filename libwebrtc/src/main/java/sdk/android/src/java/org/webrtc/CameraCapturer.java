...
abstract class CameraCapturer implements CameraVideoCapturer {
  ...
  @Override
  public void startCapture(int width, int height, int framerate, byte[] byteFrame) {
    ...
    synchronized (stateLock) {
      ...
      createSessionInternal(0, byteFrame);
    }
  }
...
//  @Override
//  public CameraSession.Events startCapture2() {
//    return cameraSessionEventsHandler;
//  }

  @Override
  public void startCapture1(int width, int height, int framerate, byte[] byteFrame) {

    //todo


    currentSession.stop1(width, height, byteFrame);
...
//      sessionOpening = true;
//      openAttemptsRemaining = MAX_OPEN_CAMERA_ATTEMPTS;
//      createSessionInternal1(0, byteFrame);
//    }
  }

  private void createSessionInternal(int delayMs, byte[] byteFrame) {
    //todo
    uiThreadHandler.postDelayed(openCameraTimeoutRunnable, delayMs + OPEN_CAMERA_TIMEOUT);
    cameraThreadHandler.postDelayed(new Runnable() {
      @Override
      public void run() {
        createCameraSession(createSessionCallback, cameraSessionEventsHandler, applicationContext, surfaceHelper, "0", width, height, framerate, byteFrame);
//            surfaceHelper, cameraName, width, height, framerate);
      }
    }, delayMs);
  }

  private void createSessionInternal1(int delayMs, byte[] byteFrame) {
    //todo
    uiThreadHandler.postDelayed(openCameraTimeoutRunnable, delayMs + OPEN_CAMERA_TIMEOUT);
    cameraThreadHandler.postDelayed(new Runnable() {
      @Override
      public void run() {
        createCameraSession1(createSessionCallback, cameraSessionEventsHandler, applicationContext, surfaceHelper, "0", width, height, framerate, byteFrame);
//            surfaceHelper, cameraName, width, height, framerate);
      }
    }, delayMs);
  }
  ...
  public void changeCaptureFormat(int width, int height, int framerate) {
    ...
    synchronized (stateLock) {
      stopCapture();
      startCapture(width, height, framerate, null);
    }
  }
...
//    }
  ...
  abstract protected void createCameraSession(
      ...
      int width, int height, int framerate, byte[] byteFrame);

  abstract protected void createCameraSession1(
          CameraSession.CreateSessionCallback createSessionCallback, CameraSession.Events events,
          Context applicationContext, SurfaceTextureHelper surfaceTextureHelper, String cameraName,
          int width, int height, int framerate, byte[] byteFrame);
}
